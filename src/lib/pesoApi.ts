...
/** Qual ritmo vira a inclinação da linha em modo "ritmo_semanal" — ver calcularLinhaMetaPorDia.
 * "media" = média entre percentualMin e percentualMax. */
export type ReferenciaIdeal = "minimo" | "media" | "maximo";

/** Estilo da linha de meta (ver calcularLinhaMetaPorDia pro algoritmo de cada um), inspirados nos
 * apps de referência do mercado:
 * - "ritmo_semanal" (estilo MacroFactor): reseta a cada 7 dias pro valor real da média daquele
 *   dia (sem acumular atraso/adiantamento) e projeta 7 dias à frente no ritmo escolhido.
 * - "faixa_alvo" (estilo TrendWeight): sem trajetória — a meta é sempre o próprio peso-alvo,
 *   constante do início ao fim; só `pesoAlvo` importa, ritmo não se aplica. */
export type EstiloMeta = "ritmo_semanal" | "faixa_alvo";
 ...
 * `percentualMin` sempre a MENOR magnitude); só usados quando `estiloMeta` é "ritmo_semanal".
 * `pesoAlvo` é o peso buscado — pro tipo manutenção é o próprio peso de manutenção; pro tipo
 * percentual é o alvo final da perda/ganho, usado pra projetar quantos dias faltam (ver
 * getDiasParaObjetivo) e como teto/piso da linha de meta (ver calcularLinhaMetaPorDia — nunca
 * ultrapassa o peso-alvo). */
export interface PesoMeta {
  tipo: "percentual" | "manutencao";
  estiloMeta: EstiloMeta;
  percentualMin: number | null;
  percentualMax: number | null;
  referenciaIdeal: ReferenciaIdeal;
...
export async function getMeta(): Promise<PesoMeta | null> {
  const { data, error } = await supabase
    .from("peso_metas")
    .select("tipo, estilo_meta, percentual_min, percentual_max, referencia_ideal, peso_alvo")
    .eq("user_id", uid())
    .maybeSingle();
  if (error) throw error;
  if (!data) return null;
  return {
    tipo: data.tipo,
    estiloMeta: (data.estilo_meta as EstiloMeta | null) ?? "ritmo_semanal",
    percentualMin: data.percentual_min,
    percentualMax: data.percentual_max,
    referenciaIdeal: (data.referencia_ideal as ReferenciaIdeal | null) ?? "minimo",
...
export async function salvarMeta(
  ...
  const usaRitmo = tipo === "percentual" && estiloMeta === "ritmo_semanal";
  const { error } = await supabase.from("peso_metas").upsert({
    user_id: uid(),
    tipo,
    estilo_meta: tipo === "percentual" ? estiloMeta : null,
    percentual_min: usaRitmo ? percentualMin : null,
    percentual_max: usaRitmo ? percentualMax : null,
    referencia_ideal: usaRitmo ? referenciaIdeal : null,
    peso_alvo: pesoAlvo,
...
export async function listMetaHistorico(): Promise<PesoMetaHistorico[]> {
  const { data, error } = await supabase
    ...
    .select("tipo, estilo_meta, percentual_min, percentual_max, referencia_ideal, peso_alvo, vigente_desde")
    .eq("user_id", uid())
    .order("vigente_desde", { ascending: true });
  if (error) throw error;
  return (data ?? []).map((l) => ({
    tipo: l.tipo as "percentual" | "manutencao",
    estiloMeta: (l.estilo_meta as EstiloMeta | null) ?? "ritmo_semanal",
    percentualMin: l.percentual_min as number | null,
    percentualMax: l.percentual_max as number | null,
    referenciaIdeal: (l.referencia_ideal as ReferenciaIdeal | null) ?? "minimo",
...
/** Um ponto por dia com peso registrado; a média móvel usa os até 7 dias anteriores (calendário,
 * não semana fechada). Compartilhada entre getMetaSemanal e a linha de meta do gráfico de Peso
 * (Peso.svelte), pra nunca divergir. */
...
function ritmoIdealDe(m: PesoMetaHistorico): number {
  if (m.percentualMin == null || m.percentualMax == null) return 0;
  if (m.referenciaIdeal === "minimo") return m.percentualMin;
  if (m.referenciaIdeal === "maximo") return m.percentualMax;
  return (m.percentualMin + m.percentualMax) / 2;
}

const DIAS_RESET_RITMO_SEMANAL = 7;

/**
 * Calcula, por dia, a "Meta Semanal" — fonte única usada tanto pelo card "Meta Semanal" quanto pela
 * linha vermelha do gráfico de Peso, pra nunca divergir uma da outra. Dois estilos possíveis
 * (`estiloMeta`, escolhido em Parametrização — ver os tipos pra descrição de cada um):
 *
 * - "ritmo_semanal" (estilo MacroFactor): a cada `DIAS_RESET_RITMO_SEMANAL` dias, a âncora reseta
 *   pro valor REAL da média naquele dia (sem carregar atraso/adiantamento acumulado de semanas
 *   anteriores) e projeta os próximos dias na inclinação escolhida (`referenciaIdeal`). Também
 *   reseta sempre que a configuração da meta muda (`vigenteDesde`). Cada "semana" é uma unidade
 *   independente — não existe reação a desvio nem tentativa de "recuperar o atraso".
 * - "faixa_alvo" (estilo TrendWeight): sem trajetória — a meta é sempre o próprio peso-alvo,
 *   constante do início ao fim da configuração.
 *
 * Nunca ultrapassa o peso-alvo em "ritmo_semanal" — direção (perda/ganho) definida pelo sinal de
 * percentualMin.
 */
export function calcularLinhaMetaPorDia(pesos: PesoRegistro[], historico: PesoMetaHistorico[]): Map<string, number | null> {
  ...
  const serie = calcularMediaMovelSerie(pesos);
  ...
  for (const ponto of serie) {
    const metaDoDia = metaNaData(historico, ponto.data);
    if (!metaDoDia) {
    ...
    if (metaDoDia.tipo === "manutencao" || metaDoDia.estiloMeta === "faixa_alvo") {
    ...
    if (metaDoDia.percentualMin == null) {
      mapa.set(ponto.data, null);
      continue;
    }
    ...
    const diasDesdeAncora =
      ancoraData != null ? Math.round((parseISODate(ponto.data).getTime() - parseISODate(ancoraData).getTime()) / 86400000) : Infinity;
    if (ancoraValor == null || ancoraData == null || mudouMeta || diasDesdeAncora >= DIAS_RESET_RITMO_SEMANAL) {
      ancoraValor = ponto.peso;
      ancoraData = ponto.data;
    }

    const ritmo = ritmoIdealDe(metaDoDia);
    const dias = Math.round((parseISODate(ponto.data).getTime() - parseISODate(ancoraData).getTime()) / 86400000);
    const bruto = ancoraValor * Math.pow(1 + ritmo / 100, dias / 7);
    const valor = limitarPeloAlvo(bruto, metaDoDia.percentualMin, metaDoDia.pesoAlvo);
    mapa.set(ponto.data, valor);
  }
  return mapa;
}

/** Peso-alvo da semana atual — o último valor da "Meta Semanal" (ver calcularLinhaMetaPorDia),
 * ancorado no dia mais recente com peso registrado (não necessariamente hoje). Pra "manutenção", é
 * o próprio peso-alvo cadastrado (constante, sem ritmo). null sem meta ou sem peso suficiente. */
